<template>
      ...
      <div class="bg-white p-6 sm:p-8 rounded-lg shadow-xl max-w-xs w-full mx-4">
        ...
        <div class="flex justify-center">
          <qrcode-vue
            ...
            render-as="svg"
          />
           <p v-else class="text-red-500 text-sm">Error: No URL provided for QR code.</p>
        </div>
  ...
  import QrcodeVue from 'qrcode.vue'; // Ensure qrcode.vue is installed
  ...
  const props = defineProps({
    ...
    size: { // Optional size prop for flexibility
        type: Number,
        default: 200
    }
  });
  ...
  // Make QR size reactive based on prop
  const qrSize = computed(() => props.size);
  
  </script>
  
  <style scoped>
  /* Optional: Add specific styles for the modal if needed */
